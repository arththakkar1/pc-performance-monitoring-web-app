...
export default function AnalyticsClient({
  ...
  const filteredResults =
    filter === "all" ? results : results.filter((r) => r.pc_id === filter);

  // Unique PCs for filter (if admin)
  const pcs = Array.from(
    new Set(
      results.map((r) => ({ id: r.pc_id, name: r.pcs?.name || r.pc_id })),
    ),
  ...
  const avgCpu =
    filteredResults.length > 0
      ? filteredResults.reduce((acc, curr) => acc + curr.cpu, 0) /
        filteredResults.length
      : 0;
  const avgRam =
    filteredResults.length > 0
      ? filteredResults.reduce((acc, curr) => acc + curr.ram, 0) /
        filteredResults.length
      : 0;
  const avgDisk =
    filteredResults.length > 0
      ? filteredResults.reduce((acc, curr) => acc + curr.disk_speed, 0) /
        filteredResults.length
      : 0;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Historical performance trends and diagnostics.
          </p>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={pcs.map((pc) => ({
                  ...
                  avg:
                    results
                      .filter((r) => r.pc_id === pc.id)
                      .reduce((s, r) => s + r.cpu, 0) /
                    (results.filter((r) => r.pc_id === pc.id).length || 1),
                }))}
              >
                <XAxis
                  dataKey="name"
