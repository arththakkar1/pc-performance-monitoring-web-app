...
export default function DeviceView({
  ...
  const currentCpu =
    liveMetrics?.cpu ?? (logs.length > 0 ? logs[logs.length - 1].cpu : 0);
  const currentRam =
    liveMetrics?.ram ?? (logs.length > 0 ? logs[logs.length - 1].ram : 0);
        ...
        <p className="text-muted-foreground max-w-sm">
          {deviceId ? (
            <>
              ...
              hasn&apos;t connected yet. Make sure the monitoring agent is
              running.
            </>
          ) : (
            <>
              No device ID is linked to your account. Please contact your
              administrator.
            </>
          )}
        </p>
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl mx-auto">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <Cpu className="w-3 h-3" /> Hardware Monitoring
            </span>
          </div>
        </div>
      ...
      <div className="bg-foreground/5 border border-foreground/10 rounded-lg p-3 flex items-center gap-3 text-xs text-foreground/80">
        ...
        <p>
          Viewing <strong>local hardware metrics</strong>. Data is only saved to
          the cloud history when an admin triggers a full diagnostic test.
        </p>
      </div>
        ...
        <Card className="border-foreground/10 bg-muted/10 shadow-none">
          ...
          <CardContent className="text-center">
            <p className="text-4xl font-black tracking-tighter">
              ...
              <span className="text-xl font-medium opacity-30">GB</span>
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Last {logs.length} data points — updates in real time
          </CardDescription>
        </CardHeader>
        <CardContent>
          {logs.length === 0 ? (
      ...
      <p className="text-center text-xs text-muted-foreground">
        You are viewing your device only. Contact your administrator for full
        access.
      </p>
    </div>
  );
