...
export default function DashboardClient({ initialPcs }: { initialPcs: PC[] }) {
  ...
  useEffect(() => {
    ...
    const pcSub = supabase.channel('table-db-changes-pcs')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'pcs' }, (payload) => {
        setPcs(current => {
          if (payload.eventType === 'INSERT') return [...current, payload.new as PC]
          if (payload.eventType === 'UPDATE') return current.map(p => p.id === payload.new.id ? payload.new as PC : p)
    ...
    const logSub = supabase.channel('table-db-changes-logs')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'logs' }, (payload) => {
        const newLog = payload.new as Log
        setLogs(current => {
          const pcLogs = current[newLog.pc_id] ? [...current[newLog.pc_id], newLog] : [newLog]
