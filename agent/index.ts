...
// Users can set PC_ID in .env or the agent will grab it via auth metadata
let pcId = process.env.PC_ID || os.hostname().toLowerCase()
...
async function registerPc() {
  const { data, error } = await supabase
    ...
    .upsert({ 
      id: pcId, 
      name: pcId, // Use the assigned ID as the name to match the web app
      status: 'online',
...
async function startAgent() {
  ...
  if (email && password) {
    ...
    if (error) {
      console.error('Failed to authenticate agent:', error.message)
    } else if (data.session?.user) {
      console.log('Authenticated agent as', email)
      if (data.session.user.user_metadata?.device_id) {
        pcId = data.session.user.user_metadata.device_id
        console.log(`Setting PC_ID from user metadata: ${pcId}`)
      }
    }
  }
